...
fprintf( stderr, "\t -edge 3 notoki sobel tate yoko square root\n" );
    ...
    fprintf(stderr,"DEFAULT PARAM. edge=%d,et=%d,bai=%f,kmon=%d,edon=%d,r=%f,g=%f,b=%f,cs=%d,gamma=%f,rgamma=%f,bgamma=%f,ygamma=%f,omh=%d,hasyori=%d,dither=%d,per=%f,pw=%f,pw2=%f,bun=%d,lpfon=%d,t=%f,a=%f,lpfk=%f\n",ptr->edge,ptr->et,ptr->bai,ptr->kmon,ptr->edon,ptr->r,ptr->g,ptr->b,ptr->cs,ptr->gamma,ptr->rgamma,ptr->bgamma,ptr->ygamma,ptr->omh,ptr->hasyori,ptr->dither,ptr->per,ptr->pw,ptr->pw2,ptr->bun,ptr->lpfon,ptr->t,ptr->a,ptr->lpfk);   
...
double AVE1,AVE2,BUN1,BUN2,AVEY1,AVEY2,BUNY1,BUNY2,AVEZ1,AVEZ2,BUNZ1,BUNZ2;
int TOTAL1,TOTAL2;
...
//AVE2 /= (float)(TOTAL2);
        ...
        BUNZ2 /= TOTAL2;
    if(omh == 3){
        HANTEI = (double)(TOTAL1)*(double)(TOTAL2)*(AVE1 - AVE2)*(AVE1 - AVE2);
    }
    if(omh == 4){
    HANTEI2 = (BUN1+BUNY1+BUNZ1) * (double)TOTAL1 + (BUN2+BUNY2+BUNZ2) * (double)TOTAL2;
    }
//    printf("HANTEI=%f\n",HANTEI);
    if(omh == 3){
        if(MAX2 < HANTEI){
...
//        double lpfk = 0.75;
    ...
    for(i=0;i<vsize;i++){
        for(j=0;j<hsize;j++){
            if((j>=1)  && (j <= (hsize-2)) ){
            YIN[j*vsize+i] = lpfk*(double)RIN[j*vsize+i]+koi*(double)RIN[(j-1)*vsize+i]+koi*(double)RIN[(j+1)*vsize+i];
            UIN[j*vsize+i] = lpfk*(double)GIN[j*vsize+i]+koi*(double)GIN[(j-1)*vsize+i]+koi*(double)GIN[(j+1)*vsize+i];
            VIN[j*vsize+i] = lpfk*(double)BIN[j*vsize+i]+koi*(double)BIN[(j-1)*vsize+i]+koi*(double)BIN[(j+1)*vsize+i];
            }
        }
        //if(i==1){while(1);}
    }
    ...
    for(i=0;i<hsize;i++){
        for(j=0;j<vsize;j++){
            if((j>=1)  && (j <= (vsize-2)) ){
            RIN[i*vsize+j] = (unsigned char)(lpfk*(double)YIN[i*vsize+j]+koi*(double)YIN[i*vsize+(j-1)]+koi*(double)YIN[i*vsize+(j+1)]+0.5);
            GIN[i*vsize+j] = (unsigned char)(lpfk*(double)UIN[i*vsize+j]+koi*(double)UIN[i*vsize+(j-1)]+koi*(double)UIN[i*vsize+(j+1)]+0.5);
            BIN[i*vsize+j] = (unsigned char)(lpfk*(double)VIN[i*vsize+j]+koi*(double)VIN[i*vsize+(j-1)]+koi*(double)VIN[i*vsize+(j+1)]+0.5);
            }
        }
    }
    } //else {
    //    for(i=0;i<hsize*vsize;i++){
    //        RrIN[i] = (double)RIN[i];//+0.5*(double)YIN[i*vsize+(j-1)]+0.25*(double)YIN[i*vsize+(j+1)];
    //        GgIN[i] = (double)GIN[i];//+0.5*(double)UIN[i*vsize+(j-1)]+0.25*(double)UIN[i*vsize+(j+1)];
    //        BbIN[i] = (double)BIN[i];//+0.5*(double)VIN[i*vsize+(j-1)]+0.25*(double)VIN[i*vsize+(j+1)];
    //    }
...
if(cs == 0){
for(i=0;i<hsize*vsize;i++){
*(YIN+i) = *(GgIN+i);//(0.29891*(float)(*(RIN+i))+0.58661*(float)(*(GIN+i))+0.11448*(float)(*(BIN+i)) /*+ 0.5*/);
*(UIN+i) = *(BbIN+i);//((-0.16874*(float)(*(RIN+i))-0.33126*(float)(*(GIN+i))+0.50000*(float)(*(BIN+i))) + 128.0);
*(VIN+i) = *(RrIN+i);//(0.50000*(float)(*(RIN+i))-0.41869*(float)(*(GIN+i))-0.08131*(float)(*(BIN+i)) + 128.0);
}
} else if(cs == 1){
for(i=0;i<hsize*vsize;i++){
RGBtoLAB(RrIN[i],GgIN[i],BbIN[i],VIN+i,YIN+i,UIN+i);//(0.29891*(float)(*(RIN+i))+0.58661*(float)(*(GIN+i))+0.11448*(float)(*(BIN+i)) + 0.5);
//*(UIN+i) = *(BIN+i);//((-0.16874*(float)(*(RIN+i))-0.33126*(float)(*(GIN+i))+0.50000*(float)(*(BIN+i))) + 128.5);
//*(VIN+i) = *(RIN+i);//(0.50000*(float)(*(RIN+i))-0.41869*(float)(*(GIN+i))-0.08131*(float)(*(BIN+i)) + 128.5);
}
} else if(cs == 2){
for(i=0;i<hsize*vsize;i++){
RGBtoLuv(RrIN[i],GgIN[i],BbIN[i],VIN+i,YIN+i,UIN+i);//(0.29891*(float)(*(RIN+i))+0.58661*(float)(*(GIN+i))+0.11448*(float)(*(BIN+i)) + 0.5);
//*(UIN+i) = *(BIN+i);//((-0.16874*(float)(*(RIN+i))-0.33126*(float)(*(GIN+i))+0.50000*(float)(*(BIN+i))) + 128.5);
//*(VIN+i) = *(RIN+i);//(0.50000*(float)(*(RIN+i))-0.41869*(float)(*(GIN+i))-0.08131*(float)(*(BIN+i)) + 128.5);
}
} else if(cs == 3 || cs == 9){
...
for(i=0;i<hsize*vsize;i++){
    if((double)(RIN[i])/255.0 >= 0.117647058823529411764705882352941){
        ...
        RrIN[i] = 255.0*pow((double)(RrIN[i])/255.0/**0.1429136476*/,1.0/2.2)*0.1429136476;
    }
    if((double)(GIN[i])/255.0 >= 0.117647058823529411764705882352941){
        GgIN[i] = 255.0*pow((double)(GgIN[i])/255.0,3.0/2.2);   
    } else {
        GgIN[i] = 255.0*pow((double)(GgIN[i])/255.0/**0.1429136476*/,1.0/2.2)*0.1429136476;
    }
    if((double)(BIN[i])/255.0 >= 0.117647058823529411764705882352941){
        BbIN[i] = 255.0*pow((double)(BbIN[i])/255.0,3.0/2.2);   
    } else {
        BbIN[i] = 255.0*pow((double)(BbIN[i])/255.0/*0.1429136476*/,1.0/2.2)*0.1429136476;
    }
   
   
   
*(YIN+i) = (0.2989*(float)(*(RrIN+i))+0.5866*(float)(*(GgIN+i))+0.1145*(float)(*(BbIN+i)) /*+0.5*/);
*(UIN+i) = ((-0.1350*(float)(*(RrIN+i))-0.2650*(float)(*(GgIN+i))+0.40000*(float)(*(BbIN+i))));
*(VIN+i) = (0.4000*(float)(*(RrIN+i))-0.3346*(float)(*(GgIN+i))-0.0653*(float)(*(BbIN+i)));
...
//                n3= (double)r*0.4000-(double)g*0.3346-(double)b*0.0653;
    ...
    for(i=0;i<hsize*vsize;i++){
        RrIN[i] = (255.0/(1.0+exp(-aaaa*((double)RrIN[i]-127.5)))-255.0/(1.0+exp(-aaaa*(-127.5))))*255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))));
        GgIN[i] = (255.0/(1.0+exp(-aaaa*((double)GgIN[i]-127.5)))-255.0/(1.0+exp(-aaaa*(-127.5))))*255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))));
        BbIN[i] = (255.0/(1.0+exp(-aaaa*((double)BbIN[i]-127.5)))-255.0/(1.0+exp(-aaaa*(-127.5))))*255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))));
        *(YIN+i) = (0.29891*(float)(*(RrIN+i))+0.58661*(float)(*(GgIN+i))+0.11448*(float)(*(BbIN+i)) /*+ 0.5*/);
        *(UIN+i) = ((-0.16874*(float)(*(RrIN+i))-0.33126*(float)(*(GgIN+i))+0.50000*(float)(*(BbIN+i))) + 128.0);
        *(VIN+i) = (0.50000*(float)(*(RrIN+i))-0.41869*(float)(*(GgIN+i))-0.08131*(float)(*(BbIN+i)) + 128.0);
    }
...
*(YIN+i) = GgIN[i];
*(UIN+i) = BbIN[i];//((-0.1350*(float)(*(RrIN+i))-0.2650*(float)(*(GgIN+i))+0.40000*(float)(*(BbIN+i))));
*(VIN+i) = RrIN[i];//(0.4000*(float)(*(RrIN+i))-0.3346*(float)(*(GgIN+i))-0.0653*(float)(*(BbIN+i)));
}
} else if(cs == 8){
for(i=0;i<hsize*vsize;i++){
    if((double)(RIN[i])/255.0 >= 0.1176470588){
...
//*(VIN+i) = RrIN[i];//(0.4000*(float)(*(RrIN+i))-0.3346*(float)(*(GgIN+i))-0.0653*(float)(*(BbIN+i)));
    ...
    if(cs == 4 || cs == 9){
    for(i=0;i<hsize*vsize;i++){   
        KARIU[i]=UIN[i]*/*cos(IRO_THIETA)*/0.941996495+VIN[i]*/*sin(IRO_THIETA)*/0.13632709;
        KARIV[i]=UIN[i]*/*sin(IRO_THIETA)*/0.118202579+VIN[i]*/*cos(IRO_THIETA)*/1.201426141;
        UIN[i]=KARIU[i];
        VIN[i]=KARIV[i];
    }
    }else {
...
for(j=0;j<hsize;j++){
if(vvv == 0){
if( (sqrt((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i]) >= et*sqrt(2.0)) ||
(sqrt((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i]) >= et*sqrt(2.0)) ||
(sqrt((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]) >= et*sqrt(2.0))
){
*(index3+j*vsize+i)=-1;
k++;
} else {
...
if(vvv == 6){
if( sqrt((((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i])) +
(((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i]) ) +
(((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]))) >= et*sqrt(6.0)
){
*(index3+j*vsize+i)=-1;
k++;
} else {
...
//printf("%f\n",sqrt((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]));
        ...
        if(vvv == 3){       
            if( (sqrt((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i]) >= et*4.0*sqrt(2.0)) ||
(sqrt((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i]) >= et*4.0*sqrt(2.0)) ||
(sqrt((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]) >= et*4.0*sqrt(2.0))
){
                *(index3+j*vsize+i)=-1;
                k++;
            } else {
...
//            printf("%f\n",sqrt(((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i])
//                +((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i])
//            +((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i])));
            if( sqrt(((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i])
                  +((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i])
            +((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i])) >= et*4.0*sqrt(6.0)){
                *(index3+j*vsize+i)=-1;
                k++;
            } else {
                *(index3+j*vsize+i)=1;
...
//printf("%f\n",sqrt((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i]));
//printf("%f\n",sqrt((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]));
//if( (sqrt((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i]) >= et*4.0) ||
//(sqrt((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i]) >= et*4.0) ||
//(sqrt((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]) >= et*4.0)
//){
//*(index3+j*vsize+i)=-1;
//k++;
//} else {
...
//    EDGERASMAX = -9999999.9e64;
    for(i=0;i<vsize;i++){
        for(j=0;j<hsize;j++){
            *(index3+j*vsize+i)=1;
            REDGE = /*sqrt*/((float)HHEDGER[j*vsize+i]*(float)HHEDGER[j*vsize+i] + (float)VVEDGER[j*vsize+i] * (float)VVEDGER[j*vsize+i]) /*/ 4.0 / sqrt(2.0)*/;
            GEDGE = /*sqrt*/((float)HHEDGEG[j*vsize+i]*(float)HHEDGEG[j*vsize+i] + (float)VVEDGEG[j*vsize+i] * (float)VVEDGEG[j*vsize+i]) /*/ 4.0 / sqrt(2.0)*/;
            REDGE = /*sqrt*/((float)HHEDGEB[j*vsize+i]*(float)HHEDGEB[j*vsize+i] + (float)VVEDGEB[j*vsize+i] * (float)VVEDGEB[j*vsize+i]) /*/ 4.0 / sqrt(2.0)*/;
//            EDGERASISAY[j*vsize+i] = 0.29891*REDGE+0.58661*GEDGE+0.11448*BEDGE;
            EDGERASISAY[j*vsize+i] = (sqrt(REDGE+GEDGE+BEDGE))/4.0/sqrt(6.0);
...
/*        int tempju;double tempuj;
        ...
        for(i=0,m=0;i<hsize*vsize;i++){
            if(*(index3+i) !=-1){
                RGB[m] = (((int)RIN[i]) << 16) | (((int)GIN[i]) << 8) | ((int)BIN[i]);
                CONVY[m] = YIN[i];
                CONVU[m] = UIN[i];
                CONVV[m] = VIN[i];
                m++;
            ...
            if (div == 0){
                ...
                for(i=0;i<hsize*vsize;i++){
                    if((*(INDEX+i)) == PT[MEN][0].INDEXNO){
                            TEMP += ((((*(YIN+i))) - U_R)*(((*(YIN+i))) - U_R) +
                            (((*(UIN+i))) - U_G)*(((*(UIN+i))) - U_G) +
                            (((*(VIN+i))) - U_B)*(((*(VIN+i))) - U_B))*(255.0-EDGERASISAY[i])/255.0;
// if( MAXD < TEMP){
// MAXD = TEMP;
// }
                    }
            ...
            if (div == 1){
                ...
                for(i=0;i<hsize*vsize;i++){
                    if((*(INDEX+i)) == PT[MEN][0].INDEXNO){
                        ...
                        TEMP += pow(((((*(YIN+i))) - U_R)*(((*(YIN+i))) - U_R) +
                            (((*(UIN+i))) - U_G)*(((*(UIN+i))) - U_G) +
                            (((*(VIN+i))) - U_B)*(((*(VIN+i))) - U_B)),pw/2.0)*pow((255.0-EDGERASISAYD)/255.0,pw2);
// if( MAXD < TEMP){
// MAXD = TEMP;
// }
...
MAXINDEXNUM = PT[MEN][i].MAXDISTANCE/**PT[MEN][i].INDEXNUM*/;//max distance tuika
NUM = i;
}
}
    } else if(bun == 0){
...
//V_JYUSHIN[i] = 255.0*rmult*pow(V_JYUSHIN[i]/(rmult*255.0),1.0/rgamma);
    ...
    if(cs == 4 || cs == 9){
    for(i=0;i<IROSUU;i++){
        KARIU[i] = U_JYUSHIN[i]/((float)(bmult))*/*cos(IRO_THIETA)*/1.076908585-V_JYUSHIN[i]/((float)(rmult))*/*sin(IRO_THIETA)*/0.122197951;
        KARIV[i] = -U_JYUSHIN[i]/((float)(bmult))*/*sin(IRO_THIETA)*/0.105951892+V_JYUSHIN[i]/((float)(rmult))*/*cos(IRO_THIETA)*/0.844366606;
        U_JYUSHIN[i] = KARIU[i];
        V_JYUSHIN[i] = KARIV[i];
        Y_JYUSHIN[i] /= gmult;
    }
    ...
    for(i=0;i<IROSUU;i++){
        KARIU[i] = U_JYUSHIN[i]/((float)(bmult))*cos(IRO_THIETA)+V_JYUSHIN[i]/((float)(rmult))*sin(IRO_THIETA);
        KARIV[i] = -U_JYUSHIN[i]/((float)(bmult))*sin(IRO_THIETA)+V_JYUSHIN[i]/((float)(rmult))*cos(IRO_THIETA);
        U_JYUSHIN[i] = KARIU[i];
        V_JYUSHIN[i] = KARIV[i];
        Y_JYUSHIN[i] /= gmult;
    }
...
if(cs == 0){
for(i=0;i<IROSUU;i++){
    FREDUCE_R[i] = 255.0*pow(((float)V_JYUSHIN[i])/255.0,1.0/rgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_G[i] = 255.0*pow(((float)Y_JYUSHIN[i])/255.0,1.0/gamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_B[i] = 255.0*pow(((float)U_JYUSHIN[i])/255.0,1.0/bgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.77200*((float)(U_JYUSHIN[i]/((float)(BMULT)))-128.0))+0.5);
...
for(i=0;i<IROSUU;i++){
LABtoRGB((float)V_JYUSHIN[i],(float)Y_JYUSHIN[i],(float)U_JYUSHIN[i],FREDUCE_R+i,FREDUCE_G+i,FREDUCE_B+i);
    FREDUCE_R[i] = 255.0*pow(((float)FREDUCE_R[i])/255.0,1.0/rgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_G[i] = 255.0*pow(((float)FREDUCE_G[i])/255.0,1.0/gamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_B[i] = 255.0*pow(((float)FREDUCE_B[i])/255.0,1.0/bgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.77200*((float)(U_JYUSHIN[i]/((float)(BMULT)))-128.0))+0.5);
    IREDUCE_R[i]=(int)(FREDUCE_R[i]+0.5);
    IREDUCE_G[i]=(int)(FREDUCE_G[i]+0.5);
    IREDUCE_B[i]=(int)(FREDUCE_B[i]+0.5);
...
for(i=0;i<IROSUU;i++){
LuvtoRGB((float)V_JYUSHIN[i],(float)Y_JYUSHIN[i],(float)U_JYUSHIN[i],FREDUCE_R+i,FREDUCE_G+i,FREDUCE_B+i);
    FREDUCE_R[i] = 255.0*pow(((float)FREDUCE_R[i])/255.0,1.0/rgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_G[i] = 255.0*pow(((float)FREDUCE_G[i])/255.0,1.0/gamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_B[i] = 255.0*pow(((float)FREDUCE_B[i])/255.0,1.0/bgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.77200*((float)(U_JYUSHIN[i]/((float)(BMULT)))-128.0))+0.5);
    IREDUCE_R[i]=(int)(FREDUCE_R[i]+0.5);
...
for(i=0;i<IROSUU;i++){
//IREDUCE_R[i] = (int)((((float)(Y_JYUSHIN[i]))+1.40200*((float)(V_JYUSHIN[i])-128.0))+0.5);
//IREDUCE_G[i] = (int)((((float)(Y_JYUSHIN[i]))-0.34414*((float)(U_JYUSHIN[i])-128.0)-0.71414*((float)(V_JYUSHIN[i])-128.0))+0.5);
//IREDUCE_B[i] = (int)((((float)(Y_JYUSHIN[i]))+1.77200*((float)(U_JYUSHIN[i])-128.0))+0.5);
FREDUCE_R[i] = ((((float)(Y_JYUSHIN[i]))+1.4018452447105*((float)(V_JYUSHIN[i])-128.0)));
FREDUCE_G[i] = ((((float)(Y_JYUSHIN[i]))-0.34511790321824*((float)(U_JYUSHIN[i])-128.0)-0.71429038997809*((float)(V_JYUSHIN[i])-128.0)));
FREDUCE_B[i] = ((((float)(Y_JYUSHIN[i]))+1.7710177314704*((float)(U_JYUSHIN[i])-128.0)));
    FREDUCE_R[i] = 255.0*pow(((float)FREDUCE_R[i])/255.0,1.0/rgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_G[i] = 255.0*pow(((float)FREDUCE_G[i])/255.0,1.0/gamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_B[i] = 255.0*pow(((float)FREDUCE_B[i])/255.0,1.0/bgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.77200*((float)(U_JYUSHIN[i]/((float)(BMULT)))-128.0))+0.5);
    IREDUCE_R[i]=(int)(FREDUCE_R[i]+0.5);
...
//} else if(cs == 4){
//for(i=0;i<IROSUU;i++){
//IREDUCE_R[i] = (int)((((float)(Y_JYUSHIN[i])*0.9998247134019967)-0.000044452970991771824*((float)(U_JYUSHIN[i]))+1.7528659800326482*((float)(V_JYUSHIN[i])))+0.5);
//IREDUCE_G[i] = (int)((((float)(Y_JYUSHIN[i])*1.0000893144198046)-0.4320813565838843*((float)(U_JYUSHIN[i]))-0.89314419804726*((float)(V_JYUSHIN[i])))+0.5);
//IREDUCE_B[i] = (int)((((float)(Y_JYUSHIN[i])*1.0000000115762946)+2.213731098385467*((float)(U_JYUSHIN[i]))-0.00011576294529099052*((float)(V_JYUSHIN[i])))+0.5);
////            RR= n1*0.9998247134019967+n2*-0.000044452970991771824+n3* 1.7528659800326482;
////            GG= n1*1.0000893144198046+n2*-0.4320813565838843+     n3*-0.89314419804726;
////            BB= n1*1.0000000115762946+n2* 2.213731098385467+      n3*-0.00011576294529099052;
//}   
//}
} else if(cs == 4){
    for(i=0;i<IROSUU;i++){
    KAARII = ((((float)(Y_JYUSHIN[i])*0.9998247134019967)-0.000044452970991771824*((float)(U_JYUSHIN[i]))+1.7528659800326482*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
    }
        FREDUCE_R[i] = KAARII;
...
//                rr=Math.Pow(r3/0.142913647595774056881018286010431,2.2)*255;
    KAARII = ((((float)(Y_JYUSHIN[i])*1.0000893144198046)-0.4320813565838843*((float)(U_JYUSHIN[i]))-0.89314419804726*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
        ...
        FREDUCE_G[i] = KAARII;
    KAARII = ((((float)(Y_JYUSHIN[i])*1.0000000115762946)+2.213731098385467*((float)(U_JYUSHIN[i]))-0.00011576294529099052*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
...
//IREDUCE_G[i] = (int)((((float)(Y_JYUSHIN[i]))-0.34414*((float)(U_JYUSHIN[i])-128.0)-0.71414*((float)(V_JYUSHIN[i])-128.0))+0.5);
//IREDUCE_B[i] = (int)((((float)(Y_JYUSHIN[i]))+1.77200*((float)(U_JYUSHIN[i])-128.0))+0.5);
//    IREDUCE_R[i] = (int)((((float)(Y_JYUSHIN[i]))+1.4018452447105*((float)(V_JYUSHIN[i])-128.0))+0.5);
//    IREDUCE_G[i] = (int)((((float)(Y_JYUSHIN[i]))-0.34511790321824*((float)(U_JYUSHIN[i])-128.0)-0.71429038997809*((float)(V_JYUSHIN[i])-128.0))+0.5);
//    IREDUCE_B[i] = (int)((((float)(Y_JYUSHIN[i]))+1.7710177314704*((float)(U_JYUSHIN[i])-128.0))+0.5);
//        RrIN[i] = (255.0/(1.0+exp(-aaaa*((double)RIN[i]-127.5)))-255.0/(1.0+exp(-aaaa*(-127.5))))*255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))));
    KAARII=((((float)(Y_JYUSHIN[i]))+/*1.4018452447105*/1.40200*((float)(V_JYUSHIN[i])-128.0)));
    KAARII=logl(255.0/((KAARII/(255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))))))+255.0/(1.0+exp(-aaaa*(-127.5))))-1.0)/(-aaaa)+127.5;
    FREDUCE_R[i] =(KAARII);
    KAARII=((((float)(Y_JYUSHIN[i]))-/*0.34511790321824*/0.34414*((float)(U_JYUSHIN[i])-128.0)-/*0.71429038997809*/0.71414*((float)(V_JYUSHIN[i])-128.0)));
    KAARII=logl(255.0/((KAARII/(255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))))))+255.0/(1.0+exp(-aaaa*(-127.5))))-1.0)/(-aaaa)+127.5;
    FREDUCE_G[i] =(KAARII);
    KAARII=((((float)(Y_JYUSHIN[i]))+/*1.7710177314704*/1.77200*((float)(U_JYUSHIN[i])-128.0))+0.5);
    KAARII=logl(255.0/((KAARII/(255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))))))+255.0/(1.0+exp(-aaaa*(-127.5))))-1.0)/(-aaaa)+127.5;
    FREDUCE_B[i] =(KAARII);
        FREDUCE_R[i] = 255.0*pow(((float)FREDUCE_R[i])/255.0,1.0/rgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_G[i] = 255.0*pow(((float)FREDUCE_G[i])/255.0,1.0/gamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
...
for(i=0;i<IROSUU;i++){
    KAARII = V_JYUSHIN[i];
KAARII=logl(255.0/((KAARII/(255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))))))+255.0/(1.0+exp(-aaaa*(-127.5))))-1.0)/(-aaaa)+127.5;
FREDUCE_R[i] = ((float)KAARII);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
    KAARII = Y_JYUSHIN[i];
KAARII=logl(255.0/((KAARII/(255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))))))+255.0/(1.0+exp(-aaaa*(-127.5))))-1.0)/(-aaaa)+127.5;
FREDUCE_G[i] = ((float)KAARII);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
    KAARII = U_JYUSHIN[i];
KAARII=logl(255.0/((KAARII/(255.0/(255.0-2.0*255.0/(1.0+exp(-aaaa*(-127.5))))))+255.0/(1.0+exp(-aaaa*(-127.5))))-1.0)/(-aaaa)+127.5;
FREDUCE_B[i] = ((float)KAARII);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.77200*((float)(U_JYUSHIN[i]/((float)(BMULT)))-128.0))+0.5);
        FREDUCE_R[i] = 255.0*pow(((float)FREDUCE_R[i])/255.0,1.0/rgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.40200*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_G[i] = 255.0*pow(((float)FREDUCE_G[i])/255.0,1.0/gamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))-0.34414*((float)(U_JYUSHIN[i])/((float)(BMULT))-128.0)-0.71414*((float)(V_JYUSHIN[i])/((float)(RMULT))-128.0))+0.5);
FREDUCE_B[i] = 255.0*pow(((float)FREDUCE_B[i])/255.0,1.0/bgamma);//(int)((((float)(Y_JYUSHIN[i])/((float)(YMULT)))+1.77200*((float)(U_JYUSHIN[i]/((float)(BMULT)))-128.0))+0.5);
    IREDUCE_R[i]=(int)(FREDUCE_R[i]+0.5);
...
//            else
//                rr=Math.Pow(r3/0.142913647595774056881018286010431,2.2)*255;
    KAARII = Y_JYUSHIN[i];//)*1.0000893144198046)-0.4320813565838843*((float)(U_JYUSHIN[i]))-0.89314419804726*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
        ...
        FREDUCE_G[i] = KAARII;
    KAARII = U_JYUSHIN[i];//))-0.00011576294529099052*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
...
//            else
//                rr=Math.Pow(r3/0.142913647595774056881018286010431,2.2)*255;
    KAARII = FREDUCE_G[i];//)*1.0000893144198046)-0.4320813565838843*((float)(U_JYUSHIN[i]))-0.89314419804726*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
        ...
        FREDUCE_G[i] = KAARII;
    KAARII = FREDUCE_B[i];//))-0.00011576294529099052*((float)(V_JYUSHIN[i]))));
    if(KAARII/255.0 >=0.0540269630587776405948631399435028){
        KAARII = pow(KAARII/255.0,2.2/3.0)*255.0;       
    }else{
        KAARII = pow(KAARII/255.0/0.142913647595774056881018286010431,2.2)*255.0;
...
// b= (*(IIBIN+x*vsize+y))+((*(errorB+adr))/16);
if(dither == 0){
r= (double)(*(VIN+x*vsize+y))+((*(errorR+adr))/(16.0/(per+0.0000001))/*32.0*/);
g= (double)(*(YIN+x*vsize+y))+((*(errorG+adr))/(16.0/(per+0.0000001))/*32.0*/);
b= (double)(*(UIN+x*vsize+y))+((*(errorB+adr))/(16.0/(per+0.0000001))/*32.0*/);
} else if(dither == 1){
r= (double)(*(VIN+x*vsize+y))+((*(errorR+adr))/(4.0/(per+0.0000001)));
g= (double)(*(YIN+x*vsize+y))+((*(errorG+adr))/(4.0/(per+0.0000001)));
b= (double)(*(UIN+x*vsize+y))+((*(errorB+adr))/(4.0/(per+0.0000001)));
} else if(dither == 2){
r= (double)(*(VIN+x*vsize+y))+((*(errorR+adr))/(42.0/(per+0.0000001)));
g= (double)(*(YIN+x*vsize+y))+((*(errorG+adr))/(42.0/(per+0.0000001)));
b= (double)(*(UIN+x*vsize+y))+((*(errorB+adr))/(42.0/(per+0.0000001)));
} else if(dither == 3){
r= (double)(*(VIN+x*vsize+y))+((*(errorR+adr))/(48.0/(per+0.0000001)));
g= (double)(*(YIN+x*vsize+y))+((*(errorG+adr))/(48.0/(per+0.0000001)));
b= (double)(*(UIN+x*vsize+y))+((*(errorB+adr))/(48.0/(per+0.0000001)));
}
/* if(r > 255.0){
r=255.0;
}
...
est = 999999.9e33;
for(i=0;i<IROSUU;i++){
ee = (r-(double)(V_JYUSHIN1[i]))*(r-(double)(V_JYUSHIN1[i]))+(g-(double)(Y_JYUSHIN1[i]))*(g-(double)(Y_JYUSHIN1[i]))+(b-(double)(U_JYUSHIN1[i]))*(b-(double)(U_JYUSHIN1[i]));
if(est>ee){
bst = i;
est = ee;
}
...
for(i=0;i<hsize*vsize;i++){
for(j=0;j<IROSUU;j++){
if( ((int)((*(IIRIN+i))/* *65384.0*/) == (int)((V_JYUSHIN1[j])/* *65384.0*/)) &&
((int)((*(IIGIN+i))/* *65384.0*/) == (int)((Y_JYUSHIN1[j])/**65384.0*/)) &&
((int)((*(IIBIN+i))/* *65384.0*/) == (int)((U_JYUSHIN1[j])/* *65384.0*/)) ){
*(PALETGAZOU+i) = j;
}
}
}
...
cnt->edge = 6;cnt->et = 20;cnt->bai = 1.0;cnt->kmon = 0;cnt->edon=2;cnt->r=4.0;cnt->g=4.0;cnt->b=3.0;
cnt->cs=3;cnt->gamma=1.1;cnt->rgamma=1.1;cnt->bgamma=1.1;cnt->ygamma=1.1;cnt->omh=4;cnt->hasyori=0.0;
cnt->dither=0;cnt->per=0.75;cnt->pw=2.0;cnt->pw2=1.0;cnt->bun=1;cnt->lpfon=0;cnt->t=5.0;cnt->a=0.001;
cnt->lpfk = 0.8;
if(argc < 2){;
print_help(cnt);
exit(0);
...
MedianCut(hsize, vsize, RIN, GIN, BIN, PALETGAZOU,REDUCE_R,REDUCE_G,REDUCE_B,cnt->edge,cnt->et,cnt->edon,cnt->kmon,cnt->r,cnt->g,cnt->b,cnt->gamma,cnt->omh,cnt->hasyori,cnt->dither,cnt->cs,cnt->per,cnt->div,cnt->pw,cnt->pw2,cnt->t,cnt->a,cnt->rgamma,cnt->bgamma,cnt->ygamma,cnt->bun,cnt->lpfon,cnt->bai,cnt->lpfk);//ROUT, GOUT, BOUT);
//debug start
// for(;;);
//debug end
